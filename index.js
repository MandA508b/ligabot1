...
bot.action('create_chat', async (ctx)=> {
    try{
        ...
        if(candidat){
            try{
                ...
                return await ctx.telegram.sendMessage(userClient.telegramId,`Ви вже відповідали на це замовлення\n\nВідповісти на замовлення №${number}`, Markup.inlineKeyboard([
                        [Markup.button.webApp(`Відповісти`, `${process.env.CHAT_URL}/chat?name=client&room=${candidat.room}&advertisementId${advertisement._id}`)],
...

        {
            ...
            let chatData = await chatDataService.create(chat.room, "LigaBot", `Оголошення №${advertisement.number}`)
            chatData = await chatDataService.create(chat.room, "LigaBot", `${advertisement.type}: ${advertisement.total}USDT trc20`)
            chatData = await chatDataService.create(chat.room, "LigaBot", `Місто: ${cityName.name}`)
            chatData = await chatDataService.create(chat.room, "LigaBot", `Частин: ${advertisement.part}`)
            chatData = await chatDataService.create(chat.room, "LigaBot", `Ставка: ${advertisement.rate}%`)
        ...
        await ctx.telegram.sendMessage(userClient.telegramId,`Відповісти на замовлення №${number}`, Markup.inlineKeyboard([
            [Markup.button.webApp(`Відповісти`, `${process.env.CHAT_URL}/chat?name=client&room=${chat.room}&advertisementId${advertisement._id}`)],
            [Markup.button.callback('Видалити чат', 'delete_chat')]
            ])
        )
        ...
        await ctx.telegram.sendMessage(userCustomer.telegramId, `Чат #${chat.number}\n\nХтось хоче вам відповісти на замовлення №${number}`, Markup.inlineKeyboard([[
                Markup.button.webApp(`Відповісти`, `${process.env.CHAT_URL}/chat?name=author&room=${chat.room}&advertisementId${advertisement._id}`)
            ],
            [
...
bot.action('send_rate_request', async (ctx)=> {
    try{
        ...
        if(userCustomer._id.toString() === userClient._id.toString()){
            return await sendMessageWithKeyboard(chatId, 'Ви не можете запропонувати ціну самі собі !')
        }
...
bot.action('accept_rate', async (ctx)=> {
    try{
        ...
        await ctx.telegram.sendMessage(userClient.telegramId,`Вашу ставку на замовлення №${advertisementNumber} одобрили`, Markup.inlineKeyboard([
                Markup.button.webApp(`Перейти до чату`, `${process.env.CHAT_URL}/chat?name=client&room=${chat.room}&advertisementId=${advertisement._id}&advertisementId${advertisement._id}`),
            ])
        )
        ...
        await ctx.telegram.sendMessage(userCustomer.telegramId, `Ви одобрили ставку на замовлення №${advertisementNumber}`, Markup.inlineKeyboard([
                Markup.button.webApp(`Перейти до чату`, `${process.env.CHAT_URL}/chat?name=author&room=${chat.room}&advertisementId=${advertisement._id}`)
            ])
        )
        ...
        for (let chatsKey in customerChats) {
            try{
                ...
                await bot.telegram.sendMessage(ctx.update.message.from.id, `Чат #${customerChats[chatsKey].number}\n\nЛистування щодо оголошення №${advertisement.number}`, Markup.inlineKeyboard([[
                    Markup.button.webApp(`Написати`, `${process.env.CHAT_URL}/chat?name=author&room=${customerChats[chatsKey].room}&advertisementId${advertisement._id}`)],
                    [
        ...
        for (let chatsKey in clientChats) {
            try{
                ...
                await bot.telegram.sendMessage(ctx.update.message.from.id, `Листування щодо оголошення №${advertisement.number}`, Markup.inlineKeyboard([[
                    Markup.button.webApp(`Написати`, `${process.env.CHAT_URL}/chat?name=client&room=${clientChats[chatsKey].room}&advertisementId${advertisement._id}`)],
                    [Markup.button.callback('Видалити чат', 'delete_chat')],
                    [Markup.button.callback('Викликати арбітраж', 'report')],
                    [Markup.button.webApp('Позначити угоду як завершену успішно', `${process.env.ADVERTISEMENT_CREATE_URL}/review?teamId1=${user.teamId}`)]])
                )
...
bot.action('fix_advertisement', async (ctx)=> {
    try{
        ...
        ctx.reply(`Чат #${numberChat} на оголошення №${advertisement.number} успішно зафіксовано!`)
...
bot.action('reserve_advertisement', async (ctx)=> {
    try{
        ...
        ctx.reply(`Чат #${numberChat} на оголошення №${advertisement.number} успішно зарезервовано!`)
...
bot.action('open_advertisement', async (ctx)=> {
    try{
        ...
        ctx.reply(`Чат на оголошення №${numberAdvertisement} успішно відкрито!`)
    ...
    for (let chatsKey in chats) {
        try{
            ...
            await bot.telegram.sendMessage(chatId, `Чат #${chats[chatsKey].number}\n\nЛистування щодо оголошення №${advertisement.number}`, Markup.inlineKeyboard([[
                Markup.button.webApp(`Написати`, `${process.env.CHAT_URL}/chat?name=author&room=${chats[chatsKey].room}&advertisementId${advertisement._id}`)],
                [
                    Markup.button.callback(`${buttonReserved}`, callbackButtonReserved),
                    Markup.button.callback(`${buttonFixed}`, callbackButtonFixed),
                ],
